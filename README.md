...
- **Road A/B experiment:** compares two equal-nominal-exposure shutter/gain arms in randomized, balanced ABBA/BAAB blocks without leaving the configured video pipeline. B gain is derived from A so shutter × analogue gain stays equal. The Pi accepts frames only after actual exposure/gain metadata matches the arm, preserves three native Y-plane samples per slot as lossless PGM, and records request metadata in an atomic manifest. A normalized road ROI must be confirmed. Eight 1080p blocks need roughly 200 MiB. Camera controls are restored afterward; the experiment never applies a winner automatically.
