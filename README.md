...
- Full-sensor JPEG stills outside recording; video-resolution snapshots during recording; full-resolution bursts of several photos per second from a continuous full-sensor stream.
...
- **Full-resolution burst:** for several full-sensor photos per second. Periodic stills switch the camera into still mode and back for every photo, which limits them to about one per second. A burst instead streams the largest sensor mode continuously (on the B0569 its only mode, 3864 × 2192 at 15.75 fps) and saves frames at the chosen rate (0.1–30 photos per second), so there is no mode switch between photos and the preview keeps running. Frames are picked by sensor timestamp, so 4 photos per second from a 15.75 fps stream saves every third or fourth frame and averages 4.0. The photo limit (0 means until stopped) and optional JSON control overrides work as for periodic stills. Advanced settings choose the JPEG quality (default 90) and the number of encoder threads (1–4, default 3). The sustainable rate depends on how fast the Pi 4 CPU encodes full-sensor JPEGs and how fast storage writes several MB per photo, so it is found by trying: while running, the status line shows the achieved rate and how many due frames were skipped because every encoder was busy. Skipped frames are never queued, so a burst cannot build an unbounded backlog. Each photo is an ordinary library still with its own request metadata (including `SensorTimestamp`), recorded as capture mode `burst`. JPEGs are encoded directly from the camera's YUV planes in the sYCC colour space that JPEG expects. Stopping finishes photos already selected, then restores the previous camera configuration and controls.
...
`camera.py` owns camera lifecycle, shared frames, controls, capture, media derivatives, and clip recovery. `app.py` owns HTTP/authentication, streaming, range/ZIP delivery, pagination, and storage protection. `storage.py` owns S3 delivery and its deduplication ledger; `power.py` owns the fixed host power commands. `static/` is a small dependency-free browser client. Capture/reconfigure operations share a lock; MJPEG clients consume the latest frame rather than accumulating queues. A two-thread software JPEG encoder handles the small preview, leaving Pi 4's hardware encoder for H.264. Full-resolution images are processed only after a capture or during a burst, not continuously. Bursts encode JPEGs on worker threads fed by a one-slot mailbox from the camera callback, so a slow encoder skips frames instead of stalling the camera.
