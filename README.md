...
- **Camera test:** automatically captures every combination of the selected shutter times and analogue gains, with configurable settling time and 1–5 photos per pair. Defaults test 500–4,000 µs (1/2000–1/250 s) at 1×, 2×, 4× and 8× gain. Up to eight values per axis are allowed; unsupported controls or out-of-range values are rejected before anything changes. Manual exposure uses the camera's advertised mode controls (or the older AE toggle). Other controls, including focus and white balance, stay as configured. Settling occurs in preview before each full-sensor capture, which also uses the normal two settling frames after mode switching. Actual sensor results may differ from requests: comparison buttons show requested and measured exposure/gain and open the photo viewer. Each JSON sidecar retains the run ID, sample number, test settings, actual metadata and sharpness grid, including after a restart.
...
- **Sequence settings and safety:** still and test settings are kept separately for the server session. Overrides apply only during the run; the previous camera configuration and explicit controls are restored on completion, cancellation, or failure. Reconfiguration, manual captures, control edits, video recording, and power actions are blocked during sequences. Stop the sequence before switching modes. Graceful server shutdown stops the worker; sequences do not automatically resume after a restart, and already completed photos remain available.
